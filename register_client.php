...
// Gestion de l'inscription si le formulaire est soumis
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Valider les champs
    if (empty($username) || empty($password) || empty($nom) || empty($prenom) || empty($direction)) {
        ...
        if ($checkStmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $insertUserStmt = $pdo->prepare("INSERT INTO utilisateurs (Username, Password, Role) VALUES (?, ?, 'client')");
            $insertUserStmt->execute([$username, $hashedPassword]);

            // Récupérer l'ID de l'utilisateur créé
            $idUtilisateur = $pdo->lastInsertId();

            // Insérer les détails du client dans la table `client`
            $insertClientStmt = $pdo->prepare("INSERT INTO client (Nom, Prenom, Direction, IdUtilisateur) VALUES (?, ?, ?, ?)");
            try {
                $insertClientStmt->execute([$nom, $prenom, $direction, $idUtilisateur]);
                echo "<script>alert('Compte client créé avec succès ! Vous pouvez maintenant vous connecter.'); window.location.href='index.php';</script>";
                exit();
            } catch (PDOException $e) {
